...
| Pulse finding identity | Sales Outreach `PUL-1E38F625` | A cross-module target-key collision can overwrite another review module's finding. This is not the same as PLAT-206; it needs a narrow identity-scoping fix and regression test. |
...
| P0 | LinkedIn `PUL-B995BF46`; `PUL-3BD9F422` | Provide a managed, auditable schema-migration route, then use it for action-to-outcome tables and immutable approved-image identity. |
...
| P1 | Sales Outreach `PUL-1E38F625` | Scope semantic finding reuse by module/ownership as well as target identity; never overwrite a different module's finding. |
