...
| Evaluation pipeline | Upwork `PUL-E67413EC`; LinkedIn `PUL-E45BE152`; LinkedIn `PUL-90D1E2C9` | Confirmed three different fixes, not one umbrella: `PUL-E67413EC` is `issue_kind: workflow_issue` with no platform blocker named — reclassified to Upwork's own Engineering Review, not this queue. `PUL-E45BE152` is done — see below. `PUL-90D1E2C9` has no structured detail row (only a `run_concerns` text line survived), so it was left uninvestigated rather than guessed at. |
| Artifact/change coverage (done) | LinkedIn `PUL-17E6F19A`; LinkedIn `PUL-7607952E` | [PLAT-230](../bugs/pulse_platform/plat-230.md): confirmed both were already settled independently — the changelog gap by PLAT-205's explicit user decision, the dashboard cursor by the mechanism it depends on (`builder/improve.html`) having been retired platform-wide before this finding could even be actioned. |
...
| P0, partially done | LinkedIn `PUL-B995BF46`; `PUL-3BD9F422` | [PLAT-221](../bugs/pulse_platform/plat-221.md) shipped the managed schema-migration route, but a code review caught that the new tool was registered and tested, yet absent from every real agent's allowlist (workflow-step, todo-task, and Workshop/Pulse paths) — fixed as a PLAT-221 follow-up, now genuinely reachable. `PUL-B995BF46` itself turned out already resolved independently (stale finding — real `action_outcome_bindings`/`matched_action_outcome_comparisons` data since 2026-08-21, filing never re-verified); reclassify/close that workflow row. `PUL-3BD9F422` remains genuinely open (`post_approval`/`image_assets` unchanged) — its migration SQL still has not been designed. Do not mark this row fully done until that migration exists and applies. |
| ~~P1~~ done | LinkedIn `PUL-E45BE152` | [PLAT-227](../bugs/pulse_platform/plat-227.md) added the missing `pre_validation` editable field to `update_evaluation_plan`, so route gating and pre-run evidence can be realigned atomically. |
| Reclassify | Upwork `PUL-E67413EC` | `issue_kind: workflow_issue`, no platform tool gap named — belongs to Upwork's own Engineering Review, not this repository's platform queue. |
| P1 (uninvestigated) | LinkedIn `PUL-90D1E2C9` | No structured detail row survives for this finding — only a `run_concerns` text line ("evaluation report aggregator dropped a valid current eval-strategy-loop verdict"). Worth checking as a genuine platform-aggregator bug in a future pass; not started here. |
...
| ~~P2~~ done | LinkedIn `PUL-61C84987` | [PLAT-229](../bugs/pulse_platform/plat-229.md) fixed the false-pass at its root: the shared JSONPath value extractor now only unwraps genuine wildcard multi-matches, never a definite path's own array value. Code review follow-up fixed a related gap in the same function: a genuine wildcard multi-match now checks every result for `value_type`, not just the first. |
