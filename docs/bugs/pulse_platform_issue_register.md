...
| [PLAT-141](pulse_platform/plat-141.md) | A tool call that Claude Code's transcript shows completing in 41ms never produced a tool_call_end store event, so the chat showed a finished command as unresolved and the compensating settle displayed a fabricated 45.4s duration; tools dispatch under sub-session ids while open calls are tracked under the parent schedule session, but the pairing hypothesis is not yet proven | unassigned | `implemented` — pairing hypothesis disproven; transcript recovery shipped, live reverify pending | coding-agent tool event emission, `internal/events/event_store.go` |
...
| [PLAT-138](pulse_platform/plat-138.md) | [PLAT-139](pulse_platform/plat-139.md) |  |  |
