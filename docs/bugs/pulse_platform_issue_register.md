...
| PLAT-063 Report pane flashed during runs | P3 | reported on the live UI | **fixed 2026-08-09 and extended 2026-08-11: the first path was a `workflowWorkspaceView` flip that unmounted the viewer; a second independent path let ordinary outer React renders reapply the HTML iframe's `srcDoc`, which Chromium can navigate. The iframe is now memoized and accepts a refresh token only from the explicit Refresh action. Live Electron observation confirmed it remains mounted across background polling. Also records that `workflow_end` and `batch_execution_end` are dead entries in the frontend COMPLETION list** |
