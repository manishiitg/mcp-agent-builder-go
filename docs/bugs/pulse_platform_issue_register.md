...
| [PLAT-180](pulse_platform/plat-180.md) | A tool call made during a retained turn was tagged with the turn_id of whichever turn FIRST launched the coding-agent process, not the retained turn's own id — because two disconnected turn-identity systems existed (agent_go's `newPlatformTurnID()`, captured once by a closure at initial `AskWithHistory` setup and never refreshed for a later retained delivery; mcpagent's own `canonicalTurnLifecycle`, created fresh per retained delivery and correctly attached to its completion event). Fixed by adding `Session.ActiveTurnID()` (mcpagent) and having agent_go's tool-call hook resolve the live turn ID at event time via `LLMAgentWrapper.currentTurnID()` instead of trusting the stale closure — structurally provider-agnostic, no per-provider patch needed | unassigned | `fixed` — the two-system mismatch is resolved (unit-level fail-before/pass-after proof, real `Session.Run`/retained-completion-watch machinery exercised); downstream consequence of the previously-wrong turn_id not assessed; not yet re-confirmed live end-to-end (no dev server running at fix time) | `agent_go/pkg/agentwrapper/llm_agent.go`, `mcpagent/agent/turn_session.go` |
