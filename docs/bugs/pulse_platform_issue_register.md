...
| [PLAT-190](pulse_platform/plat-190.md) | Proposal: hard-enforce "read the related skill before calling this tool" for plan-mutation, report, LLM-config, schedule, and secret tools — today every such rule is prompt-steering only, no runtime check. A prior hard gate (`WithDocPrecondition`/`DocReadTracker`) was deliberately removed (`9eddc087`) because it recognized only one read channel (`get_reference_doc`, since fully retired) and falsely blocked agents reading the same content through a different valid path; any new gate must recognize every current channel (`read_skill`, confirmed intrinsic on every transport, plus a legitimate direct-shell-read-of-the-projected-file path `instructions.go` explicitly sanctions) or it repeats that failure | unassigned | `open` — proposal + design only, not implemented, not scheduled; user confirmed scope should be broad ("all tools and skills") before this ticket was filed | `agent_go/pkg/agentwrapper/llm_agent.go`, new tracker (not yet created), `mcpagent/agent/agent.go` (`AgentEventListener`) |
