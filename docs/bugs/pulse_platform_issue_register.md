...
| [PLAT-118](pulse_platform/plat-118.md) | Linux `execute_shell_command` unconditionally starts every command with `unshare -m`; rootless AppArmor-hardened hosts deny that namespace operation, so every shell call fails before even `echo` runs. The Video Studio EC2 deployment reproduces it while exposing a usable Landlock LSM | unassigned | `new` (code claims independently verified 2026-08-17 — all hold; review added to the ticket noting the stderr/reporting question, the pre-existing unguarded exec path, and Landlock ABI/contract caveats) (live root cause reproduced 2026-08-17; rootless Landlock backend, capability detection, truthful preflight, and runtime reverify required) | shared workspace shell sandbox + Linux deployment health |
