...
| [PLAT-186](pulse_platform/plat-186.md) | pi-cli's MCP `directTools` (already configured `true`) never activates, forcing every pi-cli tool call through the third-party `pi-mcp-adapter`'s fragile double-JSON-encoded `mcp(tool, args)` proxy wrapper — confirmed the structural cause behind a live incident where Gemini malformed that encoding and failed a whole run. Root cause traced to source in a third dependency, not guessed: `pi-mcp-adapter`'s own `computeServerHash` hashes the MCP server's entire declared `env` object to decide cache/direct-tool-registration validity, and our own `MCP_READY_FILE` (deliberately randomized per launch to prevent stale-marker false positives) busts that hash on every single call, unconditionally. Also confirmed: moving volatile env values to the parent process doesn't work (the MCP stdio SDK only inherits a small hardcoded allowlist, not full parent env); `MCP_API_TOKEN`/`MCP_API_URL` are already stable and not contributors; `MCP_SESSION_ID` not yet ruled in or out | unassigned | `open` — RCA and fix design complete, confirmed at source level across pi-mcp-adapter and the MCP stdio SDK; not yet implemented | `mcpagent/agent/coding_agents_bridge.go`, `mcpagent/cmd/mcpbridge/main.go`, third-party `pi-mcp-adapter` (not ours) |
