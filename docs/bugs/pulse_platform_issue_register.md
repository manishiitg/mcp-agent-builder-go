...
| [PLAT-093](pulse_platform/plat-093.md) | Answered decisions were applied after the run, so the run they were meant to change had already happened | unassigned | `implemented` (structured pre-run routing + targeted Fixer shipped; runtime reverify pending) | scheduler pre-run turn sequence |
