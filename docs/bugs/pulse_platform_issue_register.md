...
| [PLAT-152](pulse_platform/plat-152.md) | Of the three interactive (tmux) coding-agent adapters, only Pi's constructs and sends native tool-call chunks (`StreamChunkTypeToolCallStart`/`End`) through its `StreamChan`; Claude Code's and Codex CLI's interactive adapters send none at all, confirmed by grepping both the type constants and the raw channel-send syntax in every file — the adapter layer that is supposed to standardize coding agents is not standardized on this contract | unassigned | `filed` — confirmed from source across all three adapters; no code change in this ticket | `multi-llm-provider-go/pkg/adapters/{claudecode,codexcli,picli}` interactive adapters |
...
| [PLAT-150](pulse_platform/plat-150.md) | [PLAT-152](pulse_platform/plat-152.md) |  |  |
