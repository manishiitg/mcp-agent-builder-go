...
| [PLAT-177](pulse_platform/plat-177.md) | A resumed Claude Code native session (workflow "substack") told the user it had lost access to plan-editing tools (`update_message_sequence_step`, `delete_schedule`, `add_todo_task_route`, `create_human_input_request`) it had used successfully earlier in the same conversation. Investigated three concrete platform explanations and ruled out all three by direct log/code evidence: native MCP tool list doesn't exist for CLI providers to begin with (all tools are HTTP-bridge/`get_api_spec`-discovered); the relaunch's system prompt was confirmed fully present, including the exact `<available_tools>` block naming every "missing" tool, in the live prompt log for the exact incident moment; and `use_code_execution_mode` is force-`true` for CLI providers at every construction site, identically before and after resume, so no mode/convention actually changed | unassigned | `open` — investigated, no platform root cause confirmed; leading theory is a model-side self-assessment error rather than a platform defect, but not proven; raw streamed turn output not yet read | `cmd/server/server.go`, `mcpagent/agent/llm_generation.go`, `mcpagent/agent/agent.go` |
