...
| [PLAT-183](pulse_platform/plat-183.md) | Proposal (not a defect): fold the standalone `multi-llm-provider-go` repo into `mcpagent` as an internal package. Confirmed via `gh search code`/`gh api search/code` that no external consumer exists — only `mcp-agent-builder-go` and `mcpagent` reference it anywhere on GitHub. Confirmed `agent_go` itself directly imports it in dozens of files, independent of `mcpagent`'s own usage, so a move changes two dependency graphs, not one | unassigned | `open` — proposal + take-care-of checklist only; explicitly not implemented or scheduled | `multi-llm-provider-go` (whole repo), `mcpagent`, `agent_go` (direct importers), deploy scripts, docs |
