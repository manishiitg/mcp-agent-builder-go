...
| [PLAT-113](pulse_platform/plat-113.md) | Session turn occupancy was decided by sessionBusy, a display flag never set for workflow turns, so every background-agent completion during a scheduled run skipped its queue and blocked on the input lane — 25 never-started synthetic turns piled behind one 5-hour turn until the idle-wait watchdog killed a healthy run | unassigned | `partially implemented` (2026-08-16: lane is now authoritative for auto-notification queueing; register-after-acquire and sessionBusy demotion pending) | session turn occupancy + auto-notification queueing |
