...
| [PLAT-139](pulse_platform/plat-139.md) | An `ICICI-BANK-PARSING-v2` workflow step finished its real work at 09:45 (statements downloaded, `logout_verified: true`) but held its caller for ~65 minutes until the stack was stopped by hand, never reporting completion; `run_metadata.json` left frozen at `"status": "running"`. The original diagnosis (Pi never emits `agent_settled`) was disproven, as were three successor theories — cause is genuinely unknown | unassigned | `open` — cause **unknown**; four explanations explicitly ruled out with evidence so they are not re-tread. Leading remaining hypothesis: a tool call that never returned (pi emits no events between `tool_execution_start`/`_end`, and this workflow runs `TOOL_EXECUTION_TIMEOUT=90m`). The 09:45 logs had rolled over before the false diagnosis was unwound — ticket lists exactly what to capture next occurrence | unidentified layer: coding-agent tool execution, the `textChan` bridge, or the scheduler poll loop |
