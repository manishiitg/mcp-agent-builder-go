...
| [PLAT-180](pulse_platform/plat-180.md) | A tool call made during a retained turn gets tagged with the turn_id of whichever turn FIRST launched the coding-agent process, not the retained turn's own id — because two disconnected turn-identity systems exist (agent_go's `newPlatformTurnID()`, captured once by a closure at initial `AskWithHistory` setup and never refreshed for a later retained delivery; mcpagent's own `canonicalTurnLifecycle`, created fresh per retained delivery and correctly attached to its completion event). Surfaced only once PLAT-179 was fixed enough for the test to reach this check; confirmed live for pi-cli and cursor-cli (mechanism is shared, provider-agnostic code, so claude-code/codex-cli are structurally exposed too but not yet confirmed) | unassigned | `open` — root cause traced to file:line, not fixed; downstream consequence of the wrong turn_id not yet assessed; fix needs to cross the agent_go/mcpagent boundary cleanly, not a per-provider patch | `agent_go/pkg/agentwrapper/llm_agent.go`, `mcpagent/agent/turn_session.go` |
