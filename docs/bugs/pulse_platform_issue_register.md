...
| [PLAT-116](pulse_platform/plat-116.md) | A live social-media contract-upgrade turn's Codex CLI genuinely completed (`task_complete` + a real `workflow.json` version bump) two minutes in, but nothing in the platform bridge ever observed it — the streaming loop stayed blocked until the scheduler's blunt 10-minute idle timeout declared the whole run `error` 17+ minutes after it had actually succeeded, and the orphaned session/goroutine was never cleaned up, leaving it showing as stuck in the UI indefinitely. Not Codex-specific — Claude Code has an equivalent native signal too; only Pi CLI (registry-only marker path) and Cursor (default structured transport) are out of scope for now | unassigned | `partially implemented` (2026-08-16: idle-timeout path now corroborates against Codex/Claude Code's own completion signal for a precise diagnostic, and unconditionally tears down the orphaned session/HTTP/browser state — tested, fail-before/pass-after verified; the deeper bridge-stall root cause remains deferred) 2026-08-18: the Pi structured-adapter "hang" was investigated and found to be **not a defect**. The claim that pi 0.84.2 never emits `agent_settled` was a logging artifact (handled events log nothing; only unhandled `agent_end` was logged) — live runs show `agent_settled` emitted as the final event, pi exiting on its own, no stale processes. Two fix attempts were made on the false premise and both undone: `@6609765` (treat `agent_end` as terminal) shipped and caused a live regression, SIGTERM'ing a still-working process, reverted by `@fd00585`; a follow-up reworking all four structured adapters around a lingering-child-holds-stdout theory was abandoned before commit (the MCP SDK gives stdio children their own pipes). No adapter change survives. The live ICICI incident is unexplained again — tracked in [PLAT-139](pulse_platform/plat-139.md) | coding-agent completion bridge + scheduler idle-wait safety net + session cleanup |
...
| [PLAT-130](pulse_platform/plat-130.md) | Stopping a schedule marks it stopped but does not stop the work: cancelTrackedExecutionsForSession/cancelBackgroundAgents only flip an in-memory status flag read by watchers -- the goroutine actually driving a message_sequence item loop has no cancel-aware context and keeps executing queued items (including side-effecting ones) after the schedule is shown as "stopped" | unassigned | `implemented` — pre-item/pre-step cancellation gates shipped; live reverify pending | `cmd/server/workflow_execution_tracker.go`, `cmd/server/session_lifecycle.go` |
...
| [PLAT-137](pulse_platform/plat-137.md) | Strategy Auditor and Goal Advisor duplicate causal/ceiling analysis while neither reliably performs the intended hidden-mechanism audit plus outside-the-plan opportunity discovery; merge them into one compaction-safe Strategic Review sequence with final-turn typed persistence and replace the one-global-experiment limit with interference-domain conflict control | Codex | `implemented` — automated verification passed; live post-restart/compaction verification pending | Pulse module registry/Gate, strategic Review+Fix dispatch and guidance, typed persistence, experiment lifecycle, Pulse UI projection |
| [PLAT-138](pulse_platform/plat-138.md) | Engineering Review already requires prior-fix verification and backlog drainage, but buries verification, discovery, Stores/Operations evidence, deduplication, fixing and persistence inside one oversized contract with no compaction-safe sequence memory; make those responsibilities explicit ordered messages with continuous semantic deduplication and final-turn typed persistence | Codex | `implemented` — automated verification passed; live post-restart/compaction verification pending | Pulse Review+Fix message-sequence dispatch/guidance, run-scoped checkpoints, typed persistence and recovery reconciliation |
| [PLAT-139](pulse_platform/plat-139.md) | An `ICICI-BANK-PARSING-v2` workflow step finished its real work at 09:45 but held its caller ~65 minutes, never reporting completion. Four early theories (incl. Pi never emits `agent_settled`) were disproven | unassigned | `fixed` (2026-08-18: root cause confirmed live via a production pprof goroutine dump on a second same-day occurrence, `check-form-26as-xspaces`. `cmd.Stderr = &bytes.Buffer{}` hands `os/exec` an internal pipe the adapter has no handle to; `cmd.Wait()` blocks until that pipe's own copy goroutine reaches EOF too, not just process reap — stdout was fixed this morning via `cmd.StdoutPipe()`, stderr never was. Fixed symmetrically via `cmd.StderrPipe()`, `multi-llm-provider-go@6d8e4e9`. Hermetic test's pre-fix failure produces the identical stack trace as the production dump — a reproduction, not a guess. Still open: what actually held stderr open is unidentified for both occurrences) | `multi-llm-provider-go` picli structured adapter |
| [PLAT-142](pulse_platform/plat-142.md) | PLAT-116 orphan cleanup closed the Build-in-Public Pulse conversation after Gate timed out, then the scheduler immediately sent Review+Fix into that closed session, stranding the remaining sequence and later schedules | unassigned | `open` — live RCA confirmed | per-turn lifecycle + scheduled conversation continuation |
| [PLAT-143](pulse_platform/plat-143.md) | Workflow restoration is represented by one global frontend boolean, so hydration of another session can cover LinkedIn with a repeatedly re-armed `Restoring previous session…` screen while LinkedIn's own APIs are healthy | unassigned | `open` — state-ownership flaw confirmed | keyed frontend restore lifecycle |
| [PLAT-144](pulse_platform/plat-144.md) | Logically dependent schedules can only approximate ordering with cron offsets; Tectonic Daily Pulse fired 15 minutes after close, while close normally required 22+ minutes, and collided | unassigned | `design_required` | schedule dependencies + overlap validation |
| [PLAT-145](pulse_platform/plat-145.md) | A busy workflow lease terminally discards every occurrence as `skipped_busy`; three Tectonic occurrences were durably observed and then permanently lost because no queue/coalesce/retry policy exists | unassigned | `design_required` | durable scheduler collision policy |
| [PLAT-146](pulse_platform/plat-146.md) | Manual launch preserves schedule prose but no typed safety mode; the Tectonic “market close” schedule launched before cutoff could reach ordinary entry logic because close-only existed only as a time assumption | unassigned | `open` — unsafe path confirmed, no external action observed | typed schedule inputs + side-effect enforcement |
| [PLAT-147](pulse_platform/plat-147.md) | The managed backup contract allowed a generated Git bundle to be tracked inside its own source repository; Tectonic reached a ~67 GB bundle and ~133 GB `.git` through recursive self-inclusion | unassigned | `open` — live RCA confirmed, cleanup not attempted | backup destination validation + health |
...
| [PLAT-146](pulse_platform/plat-146.md) | [PLAT-147](pulse_platform/plat-147.md) |  |  |
