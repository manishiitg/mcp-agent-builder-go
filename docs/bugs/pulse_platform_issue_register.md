...
| [PLAT-181](pulse_platform/plat-181.md) | CDP shared-browser tab quota (`MaxCDPTabsPerOwner`, default 4) can be misattributed across unrelated workflows. CDP mode intentionally shares one browser connection identity (`"shared-cdp-<port>"`) across every workflow on that port — confirmed live, multiple distinct workflow sessions (upwork_harvest, publish-check, main, review-measure) all remapped to it today. `cdpOwnerID`'s fallback chain can land on that same shared string when a step's own session was never bound via `bindWorkshopBrowserSession` (which only binds at the group-level session, not necessarily the session the executing step/route actually runs under), collapsing multiple workflows onto one owner key so their tab counts sum together. Live: Instagram's `route-generate-illustrations` was blocked by "already has 4 labeled tab(s)" while owning zero of them — the 4 belonged to Upwork, Apollo, and WhatsApp | unassigned | `open` — mechanism confirmed by code and partial live evidence; the exact incident's `cdpOwner` value could not be read directly since the server restarted before the log window could be checked; two fix directions proposed, not implemented | `pkg/browser/cdp_tabs.go`, `pkg/browser/cdp_registry.go`, `pkg/browser/executor.go`, `pkg/orchestrator/agents/workflow/step_based_workflow/controller.go`, `controller_batch_execution.go` |
