...
| [PLAT-178](pulse_platform/plat-178.md) | `conversation_history` is only saved when a full `/api/query` turn completes; once a resumed session is warm (its tmux pane already alive — the normal post-resume state), further messages go through `/api/live-input`, whose only persistence hook (`appendLiveInputToPersistedChatHistory`) appends the outgoing human message but never the CLI's assistant reply. When that tmux pane later dies, restore's `persisted_snapshot` fallback tier trusts that stale JSON with no attempt at richer recovery. Live on "substack" (session b5e39872...): the snapshot froze at 13:51:26 while the user kept working (confirmed via CDP browser calls and terminal polling through 14:31); by the next restore at 16:18:20 the tmux pane was gone and the user was shown the 13:51:26 state as current. The full missing conversation is not actually lost — Claude Code's own transcript JSONL had 155 entries (36 user, 63 assistant) after 13:51:26 that the restore path never reads | unassigned | `open` — root cause fully traced to file:line via direct evidence (JSON mtime, native transcript content); fix needs two independent changes (capture assistant replies from live-input turns; have persisted_snapshot restore consult the native CLI transcript) before implementation | `cmd/server/server.go`, `cmd/server/chat_history_persistence.go`, `cmd/server/chat_history_routes.go`, `multi-llm-provider-go/claudecode_transcript_messages.go` |
