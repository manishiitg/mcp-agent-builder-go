...
| [PLAT-130](pulse_platform/plat-130.md) | Stopping a schedule marks it stopped but does not stop the work: cancelTrackedExecutionsForSession/cancelBackgroundAgents only flip an in-memory status flag read by watchers -- the goroutine actually driving a message_sequence item loop has no cancel-aware context and keeps executing queued items (including side-effecting ones) after the schedule is shown as "stopped" | unassigned | `implemented` — 2026-08-20 live reverify reproduced it again via a third mechanism: handleQuery's "Active-tab auto-resume"/"Materialize guard" blocks relaunched a killed coding-agent tmux and ran a real run_full_workflow turn after Stop. Now gated on isSessionMarkedStopped too; reverify of this fix still pending | `cmd/server/workflow_execution_tracker.go`, `cmd/server/session_lifecycle.go`, `cmd/server/server.go` |
