...
| [PLAT-133](pulse_platform/plat-133.md) | The P0 coding-agent contract enforces that a certification has a registered proof, but never that the proof tests what the certification claims. `CertStructuredMultiTurn`'s contract says a persistent provider must prove its session "survives process exit" — yet 3 of 4 providers (Codex, Cursor, Pi) registered resume-only proofs against it and passed identically to Claude, whose proof does test termination. That is why the Pi hang (PLAT-116, fixed in multi-llm-provider-go@da13e17) sailed through a contract that already required the property | unassigned | `open` (designed, deliberately not implemented: inventing fake CLI streams for Codex/Cursor would produce green-but-meaningless proofs — the exact defect — and landing the requirement before they exist turns the contract test red on every PR) | `multi-llm-provider-go` certification framework (`coding_agent_certification.go`, `coding_agent_contract.go`) |
...
| [PLAT-138](pulse_platform/plat-138.md) |  |  |  |
