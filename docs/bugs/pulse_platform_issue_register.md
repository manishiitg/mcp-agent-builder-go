...
| [PLAT-179](pulse_platform/plat-179.md) | A retained (tmux-delivered) coding-agent turn is declared complete the instant any non-empty assistant text appears in the transcript — even explicit intermediate commentary a prompt labeled "not your final answer" — with no check for a pending tool call or genuine turn end. Affects every tmux-backed provider using retained/live-steer delivery (claude-code, codex-cli, cursor-cli, agy-cli, pi-cli), not one. Live: an IC-11 retained turn completed in 2.2s with a progress-update as its final_result, before the tool's own `sleep 2` could finish. Root cause: `ReadRetainedTurnMessages` returned the last assistant message the moment it had any content; pi's own transcript can't distinguish "one message finished" from "the whole exchange is over," but pi's own status line can, and the ordinary non-retained turn flow already trusts exactly that signal (`piPaneReadyForInput`) — the retained reader simply never consulted it | unassigned | `fixed` (pi-cli only) — `ReadRetainedTurnMessages` now withholds its result until the pane confirms idle, reusing the existing `piPaneReadyForInput` check; fails closed on a capture error; fail-before/pass-after tests; claude-code/codex-cli/cursor-cli not yet audited for the same gap; live reverify pending | `multi-llm-provider-go/pkg/adapters/picli/picli_retained_turn.go` |
...
| [PLAT-178](pulse_platform/plat-178.md) | [PLAT-179](pulse_platform/plat-179.md) |  |  |
