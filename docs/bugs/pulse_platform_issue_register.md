...
| [PLAT-184](pulse_platform/plat-184.md) | Pulse and the Workflow Builder cannot access the cost ledger (`_system/costs.sqlite`) at all — confirmed via folder-guard scoping, `_system/` sits outside every workflow's read scope regardless of role, and no agent-facing tool exists to compensate (unlike Pulse's own state, `_system/pulse-platform.sqlite`, which has a whole family of dedicated tools). Consequence: PLAT-166/167's phase/item-level cost breakdown is entirely invisible to Pulse's own `model_cost_fitness` review lens and PLAT-090's "measure pulse cost" requirement. Migration scoping found the fix is more tractable than expected once two false leads were ruled out: an empty per-workflow `costs.sqlite` (confirmed stray, not a migration attempt) and an apparent 54%-unattributed-rows finding (confirmed dead historical JSONL-import debt frozen since 2026-07-12, already documented elsewhere, not live) | unassigned | `open` — root cause and migration scope confirmed via direct writer/reader catalog against the live 28k-row ledger; two fix directions proposed (per-workspace ledger migration vs. a dedicated cost-query tool mirroring the Pulse-state pattern), neither designed or implemented | `pkg/costledger`, `pkg/costobserver`, `cmd/server/server.go`, `cmd/server/cost_routes.go`, `cmd/server/virtual-tools/tool_costs.go` |
