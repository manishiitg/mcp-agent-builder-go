...
| [PLAT-067](pulse_platform/plat-067.md) | Do not dispatch a scheduled continuation to a missing/unready parent coding-agent transport | unassigned | `open` | scheduler parent-session recovery and background completion delivery |
...
| PLAT-067 Scheduled parent continues after its coding-agent terminal disappears | P0 | RTS Latency cron run, 2026-08-10 | **open — the Notion child completed, but live completion delivery timed out and the parent tmux server had already disappeared. The restored Claude prompt timed out after 5 minutes; the scheduler nevertheless dispatched the next turn, which sat until cancellation. PLAT-004 correctly made the run an error, but does not prevent this wasted/dead continuation.** |
