...
| [PLAT-115](pulse_platform/plat-115.md) | Gate/Review+Fix/Finalize always ran inside the same session as the run they reviewed — the root shape behind PLAT-113 and PLAT-114 on frequently-run workflows. Added an opt-in periodic mode: lightweight backup+notify every run, full review decoupled onto its own schedule and session, reviewing a Gate-reasoned backlog instead of one pinned folder | unassigned | `implemented` (platform capability shipped and tested; no real workflow has adopted it yet) | scheduler Pulse orchestration, workflow manifest, schedule tools |
| [PLAT-116](pulse_platform/plat-116.md) | A live social-media contract-upgrade turn's Codex CLI genuinely completed (`task_complete` + a real `workflow.json` version bump) two minutes in, but nothing in the platform bridge ever observed it — the streaming loop stayed blocked until the scheduler's blunt 10-minute idle timeout declared the whole run `error` 17+ minutes after it had actually succeeded, and the orphaned session/goroutine was never cleaned up, leaving it showing as stuck in the UI indefinitely. Not Codex-specific — Claude Code has an equivalent native signal too; only Pi CLI (registry-only marker path) and Cursor (default structured transport) are out of scope for now | unassigned | `partially implemented` (2026-08-16: idle-timeout path now corroborates against Codex/Claude Code's own completion signal for a precise diagnostic, and unconditionally tears down the orphaned session/HTTP/browser state — tested, fail-before/pass-after verified; the deeper bridge-stall root cause remains deferred) | coding-agent completion bridge + scheduler idle-wait safety net + session cleanup |
| [PLAT-117](pulse_platform/plat-117.md) | Video Studio voice dictation (sherpa-onnx-go + Nemotron streaming STT, capability-gated via `agentprofiles.RuntimeCapabilities.Voice`, SparkQuill-parity UX) shipped and passed two independent non-mic proofs (WAV file, synthetic tone), but no real-speech end-to-end pass has succeeded on the one machine it's been tested on | unassigned | `implemented_pending_live_reverify` (blocked on PLAT-119) | `pkg/voicestt`, `cmd/server/voice_stt_routes.go`, `frontend/src/voice/*` |
| [PLAT-118](pulse_platform/plat-118.md) | SparkQuill's release process was already split into its own CI workflow with its own tag namespace and path-scoped triggers, contrary to how it was carried as open on the task list; 5 most recent `main` CI runs are green | unassigned | `implemented` (CI-verified; no further work identified) | `.github/workflows/sparkquill-desktop.yml` |
| [PLAT-119](pulse_platform/plat-119.md) | A dev machine's real microphone reads exact digital silence (`rms=0.0000`) through every app, not just this one — full elimination chain ruled out the STT pipeline, wrong device, browser-automation fake device, SparkQuill holding the device, and third-party audio HAL drivers | unassigned | `open` (environment-level, not code-owned; blocks PLAT-117's live reverify) | none identified yet — OS/driver layer; see `voice_dictation_mic_captures_silence.md` |
