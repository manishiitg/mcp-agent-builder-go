...
| [PLAT-199](pulse_platform/plat-199.md) | Pulse discarded Technical Review context into a fresh Fixer and stretched one repair across repeated expensive passes | Codex | `implemented` — one retained Technical Maintenance sequence, child-session receipt identity, and backend review-phase tool gate pass focused tests as of 2026-08-28; live Pulse reverify pending | scheduled Pulse dispatch, background sequence runtime, receipt identity, tool authorization |
| [PLAT-200](pulse_platform/plat-200.md) | `harness:oversized-tool-result-recovery` verified two separable claims: (1) `--depth <n>` is a genuine no-op on a wide/flat page — live-reproduced against real Chrome (500-sibling test page: byte-identical 24,256 bytes at `--depth 2` vs. unlimited; `--depth 1` alone shrank it), matching the finding's own byte-identical-retry evidence exactly; `--depth` works as documented, it just can't help when the bloat is sibling-count, not nesting. (2) The oversized-snapshot truncation path discarded the excess entirely — `Executor` had no workspace-path field at all, so nothing could persist it; the only recovery was a full second-cost re-run via `--full-snapshot` | unassigned | `fixed` — banner now explains *why* to pick a retry option instead of listing them in try-order, and flags `--depth` as ineffective on wide/flat trees; new `Executor.SpillOversized` hook persists the full tree when configured, banner then points at `read_workspace_file(...)` instead of only a re-run. Production wiring never invents a write location: it reads the session's own already-granted `ReadPaths` (PLAT-078's `tool_output_folder` grant) and declines cleanly if absent; the write itself goes through the same `workspace.Client.UpdateWorkspaceFile` HTTP path (not a raw local file write) every other workspace write uses, scoped via the existing `WithSystemManagedWritePaths` trusted-write exception. 8 new tests (4 executor-level, 4 HTTP-level via `httptest`); full suite 19 pre-existing failures unchanged, none in touched packages | `agent_go/pkg/browser/executor.go`, `agent_go/pkg/browser/cdp_tabs_test.go`, `agent_go/cmd/server/virtual-tools/workspace_browser_tools.go`, `agent_go/cmd/server/virtual-tools/workspace_browser_tools_test.go` |
...
| [PLAT-202](pulse_platform/plat-202.md) | `harness:diff_patch_workspace_file` (2026-08-23, `knowledgebase/notes/_index.json`) reported `applied:true` while a two-hunk diff's second hunk left four scalar fields at their old values and deleted a trailing `title` line with no replacement — near-verbatim the same shape PLAT-192 fixed one day later for a different file | unassigned | `resolved` — no new code needed. Reproduced the incident's own reported before/after content directly (not the internal mechanism, which PLAT-192 already documented as unconfirmed) and confirmed PLAT-192's already-shipped `verifyDiffApplied` line-count-delta check catches it: diff claims net +1, reported broken result only changed by +0, correctly flagged as an error. New test uses the incident's own reported values as fixture data | `workspace/handlers/diff_patch_test.go` |
...
| [PLAT-202](pulse_platform/plat-202.md) | [PLAT-203](pulse_platform/plat-203.md) | [PLAT-204](pulse_platform/plat-204.md) |  |
