...
| [PLAT-130](pulse_platform/plat-130.md) | Stopping a schedule marks it stopped but does not stop the work: cancelTrackedExecutionsForSession/cancelBackgroundAgents only flip an in-memory status flag read by watchers -- the goroutine actually driving a message_sequence item loop has no cancel-aware context and keeps executing queued items (including side-effecting ones) after the schedule is shown as "stopped" | unassigned | `open` -- root cause confirmed layer by layer (scheduler ctx propagation, item-loop error handling, and askWithHistory's per-turn ctx.Err() check are all correct; the break is one layer below, at the tracked-execution cancel call); fix not started | `cmd/server/workflow_execution_tracker.go`, `cmd/server/session_lifecycle.go` |
...
| [PLAT-134](pulse_platform/plat-134.md) | [PLAT-135](pulse_platform/plat-135.md) |  |  |
