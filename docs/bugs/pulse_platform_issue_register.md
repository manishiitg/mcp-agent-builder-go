...
| [PLAT-111](pulse_platform/plat-111.md) | Cost Analysis blocks first paint on an unbounded all-history ledger/file scan and then serially loads logs for every historical run | unassigned | `open` (root cause verified 2026-08-16; bounded summary/detail API and lazy frontend loading designed) | cost API + ledger rollups + `CostsPopup` lazy detail loading |
