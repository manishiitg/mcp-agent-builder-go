...
- `builder/improve.html`: the **generated Pulse executive journal and publishable history** — Bug/Goal verdicts, one status sentence, three Latest Pulse cells, and at most six material Activity transitions. It remains required, but reviewers and the Fixer read SQLite for lifecycle truth and the Pulse popup owns reviewer coverage, assumptions, issues, backlog counts, and complete operational detail. See `read_skill(skills=[{"name":"builder-reference","path":"references/review-improve-log.md"}])` for the render contract.
