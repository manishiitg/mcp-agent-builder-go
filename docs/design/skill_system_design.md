...
> Video Studio (`agent_go/internal/videoproduct`) lives on the `feature/video-product` branch, not `main`. It is the motivating consumer for D1; everything this doc proposes changes code that is on `main`.
