...
describe('sectionThatGrew', () => {
  it('scrolls to the section that just grew', () => {
    ...
    // eye there.
    expect(sectionThatGrew({ videos: 1, characters: 0, documents: 2 }, { videos: 1, characters: 1, documents: 2 })).toBe('characters')
  })

  it('prioritizes characters over documents over videos when several grow at once', () => {
    expect(sectionThatGrew({ videos: 0, characters: 0, documents: 0 }, { videos: 1, characters: 1, documents: 1 })).toBe('characters')
    expect(sectionThatGrew({ videos: 0, characters: 1, documents: 0 }, { videos: 1, characters: 1, documents: 1 })).toBe('documents')
  })
  ...
  it('does nothing on the first known state, even if every section already has content', () => {
    ...
    expect(sectionThatGrew(null, { videos: 3, characters: 2, documents: 4 })).toBeNull()
  })

  it('does nothing when nothing grew', () => {
    expect(sectionThatGrew({ videos: 2, characters: 1, documents: 1 }, { videos: 2, characters: 1, documents: 1 })).toBeNull()
  })

  it('does nothing when a count only shrinks', () => {
    expect(sectionThatGrew({ videos: 2, characters: 1, documents: 1 }, { videos: 1, characters: 1, documents: 1 })).toBeNull()
  })
})
