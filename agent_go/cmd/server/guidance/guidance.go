...
var referenceKinds = map[string]kindMeta{
	...
	"pulse-review-fixer":    {Group: "system", Description: "Agent-owned Review then independent Fix contract: classify observations and deduplicate roots in reviewer sequences, then give a bounded compatible repair batch to a fresh Fixer and persist typed lifecycle state.", Modes: []string{"workshop", "run"}},
	...
	"prompt-engineering":    {Group: "system", Description: "How to write an optimized step description: earn every word, name the exact object shape instead of a bare category, state the outcome rather than a micromanaged procedure unless the task genuinely needs a fixed sequence, reference shared stores instead of restating their contents, and never duplicate the same paragraph across steps. A self-check before finalizing any description. Load before writing or editing any step's description.", Modes: []string{"workshop"}},
