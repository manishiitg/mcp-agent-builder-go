...
var allKinds = map[string]kindMeta{
	...
	"design-reporting-ui": {Group: "report", Description: "Design the reporting UI from scratch: author one or more standalone db/reports/*.html pages (live data via window.report; one page is selected at a time in the Report tab).", Modes: []string{"workshop"}},
...
var referenceKinds = map[string]kindMeta{
	...
	"reporting-policy":      {Group: "system", Description: "HTML-only report contract: every db/reports/*.html file is one top-level report page, labeled by <title> and optionally ordered by <meta name=report-order>. HTML reads db/db.sqlite through window.report. Covers authoring, scrolling, refresh, diagnosis, and the Run-mode boundary. Load when the user mentions reports, dashboards, themes, or layouts.", Modes: []string{"workshop", "run"}},
