...
var referenceKinds = map[string]kindMeta{
	...
	"stores":                {Group: "system", Description: "Persistent store design contract: skill vs knowledgebase vs db, when to write to which", Modes: []string{"workshop", "run"}, Tools: []string{"query_workflow_db", "mutate_workflow_db"}},
	...
	"pulse-gate":            {Group: "system", Description: "Focused scheduler Pulse Gate contract: progressive evidence scan, concerns and cadence classification, complete four-perspective worklist, compact durable handoff, and no review/fix/finalizer work.", Modes: []string{"workshop", "run"}},
	"pulse-review-fixer":    {Group: "system", Description: "Agent-owned Review+Fix contract: read Gate's durable worklist, choose the cheapest sufficient analysis, reconcile existing findings before discovery, own any specialist children, apply compatible repairs, verify prior changes, and record one terminal typed receipt for every due perspective.", Modes: []string{"workshop", "run"}},
	...
	"pulse-bug-review":      {Group: "system", Description: "The Engineering Review runtime/logic evidence pack: Exploratory QA behavioral-contract and risk-matrix method, control-path reachability checks (wrong_store_write / shadow_store_drift / dead_configuration), observable execution-trace review, and finding classifications (correctness_bug, efficiency_or_coaching, no_issue, insufficient_evidence). Gate decides whether workflow_review is due and records that in the durable worklist. Load when Engineering Review selects runtime or logic evidence.", Modes: []string{"workshop", "run"}},
	"strategy-auditor":      {Group: "system", Description: "The independent read-only Strategy Auditor contract loaded only when strategy_auditor is due: improve the current strategic shape by reconstructing its plan-to-goal causal chain, analyzing retained cross-run outcomes, detecting proxy optimization, repetition, concentration, saturation, exploration gaps, and missing causal stages, and returning bounded in-plan recommendations. Goal Advisor separately explores materially different approaches.", Modes: []string{"workshop", "run"}},
	"fix-verification":      {Group: "system", Description: "The single contract for verifying that a bounded repair actually worked, shared by scheduled Pulse, /engineering-review, and Goal Advisor: the post-change evidence boundary (baseline artifacts are never proof), what counts as valid verification (a side-effect-free deterministic check through the real runtime consumer path, or a fresh post-mutation run/eval/report artifact with matching provenance — a successful write, file existence, or mtime alone is not proof), and the changed_unverified / awaiting_next_valid_run rule when proof needs a future run. Load before applying any fix.", Modes: []string{"workshop", "run"}},
