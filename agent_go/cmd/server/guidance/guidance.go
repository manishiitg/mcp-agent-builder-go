...
var allKinds = map[string]kindMeta{
	...
	"pulse-fixer":         {Group: "improve", Description: "Apply and verify an agent-selected bounded repair batch from canonical Pulse issues, either manually or as the receipt-gated repair phase after Technical Review", Modes: []string{"workshop"}},
...
var referenceKinds = map[string]kindMeta{
	...
	"pulse-review-fixer":    {Group: "system", Description: "Agent-owned receipt-gated Review+Fix contract: classify observations and deduplicate roots in read-only review messages, then unlock a bounded compatible repair message in the same retained technical executor and persist typed lifecycle state.", Modes: []string{"workshop", "run"}},
