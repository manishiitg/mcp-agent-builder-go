...
var referenceKinds = map[string]kindMeta{
	...
	"pulse-review-fixer":    {Group: "system", Description: "Agent-owned Review+Fix contract: construct the due Engineering/Stores/Ops and Strategic Review message sequences, verify retained findings before discovery, continuously deduplicate semantic roots, apply compatible repairs, and let each final child turn record typed lifecycle state and terminal receipts.", Modes: []string{"workshop", "run"}},
	...
	"fix-verification":      {Group: "system", Description: "The single contract for verifying that a bounded repair actually worked, shared by scheduled Pulse, /engineering-review, and Strategic Review: the post-change evidence boundary (baseline artifacts are never proof), what counts as valid verification (a side-effect-free deterministic check through the real runtime consumer path, or a fresh post-mutation run/eval/report artifact with matching provenance — a successful write, file existence, or mtime alone is not proof), and the changed_unverified / awaiting_next_valid_run rule when proof needs a future run. Load before applying any fix.", Modes: []string{"workshop", "run"}},
