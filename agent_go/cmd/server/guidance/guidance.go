...
var referenceKinds = map[string]kindMeta{
	...
	"plan-drift-review":     {Group: "system", Description: "Focused scheduler plan_drift_review contract: for every step with a null drift_review record, reconcile Go-precomputed deterministic checks with the remaining judgment checks (description accuracy, learnings/KB content staleness and access appropriateness, DB normalization, and for routing-typed steps only: no shared downstream steps between sibling routes, paired with an eval), persist the merged per-step result, and hand off any real finding rather than repairing it in this turn.", Modes: []string{"workshop", "run"}},
