...
var referenceKinds = map[string]kindMeta{
	...
	"review-improve-log":           {Group: "system", Description: "The lightweight published Pulse journal spec: Bug/Goal verdicts, one status sentence, three Latest Pulse cells, at most six material Activity transitions, archive, and migration rules. The Pulse popup owns reviewer coverage, assumptions, issue lists, evidence, verification, costs, diagnostics, and filtering. Load before writing builder/improve.html from any /review-* or /improve-* skill or Pulse.", Modes: []string{"workshop", "run"}},
	"review-improve-log-migration": {Group: "system", Description: "One-time migration contract for an OLD-FORMAT builder/improve.html: how to detect old format and what must be rewritten before anything is appended. Extracted from review-improve-log so a steady-state Pulse write does not carry migration detail it never uses. Load ONLY when the existing log is old-format.", Modes: []string{"workshop", "run"}},
	"review-improve-log-skeleton":  {Group: "system", Description: "Copy-paste lightweight HTML skeleton for builder/improve.html, including the executive Pulse hierarchy, compact current-work counts, mobile-first CSS, material Activity examples, archive links, and the stable newest-first insertion anchor. Load only when creating a new Pulse journal or doing the format upgrade required by review-improve-log.", Modes: []string{"workshop", "run"}},
