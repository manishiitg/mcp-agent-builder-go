...
var referenceKinds = map[string]kindMeta{
	...
	"pulse-gate":                  {Group: "system", Description: "Focused scheduler Pulse Gate contract: progressive evidence scan, concerns and cadence classification, complete ten-module worklist, compact durable handoff, and no review/fix/finalizer work.", Modes: []string{"workshop", "run"}},
	"pulse-review-fixer":          {Group: "system", Description: "Focused scheduler consolidated Pulse review and single-fixer contract: max-two synchronous read-only reviewers, dated per-module persisted results, conflict/finding reconciliation, sequential verified fixes, and terminal module results.", Modes: []string{"workshop", "run"}},
	"pulse-finalizer":             {Group: "system", Description: "Focused scheduler Pulse finalizer contract: require terminal due-module results, then dashboard/questions, backup, publish, and notify in one ordered truthfully recorded turn.", Modes: []string{"workshop", "run"}},
