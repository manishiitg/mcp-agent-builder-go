...
var referenceKinds = map[string]kindMeta{
	...
	"pulse-review-fixer":           {Group: "system", Description: "Focused scheduler contract for independent Strategy/Goal reviews plus one shared Engineering/Ops review-and-fix sequence: reconcile existing findings before discovery, persist the review before mutation, verify prior changes, apply compatible repairs sequentially, and use a residual Fixer only for non-terminal modules.", Modes: []string{"workshop", "run"}},
