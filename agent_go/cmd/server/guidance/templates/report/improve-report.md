...
8. **Activity/actions record.** Does the report include one section — its own tab, panel, or anchored region — that plainly answers "what did this workflow actually do": recent runs and the actions taken in each, in plain non-technical language, with no raw JSON, internal IDs, or state codes? Name it for the workflow's real run cadence (`Daily Action` for a daily workflow, `Recent Activity`/`Latest Run` otherwise). Its absence, staleness (not reflecting recent runs), or fabricated content is a concrete defect, not a cosmetic nit — flag it the same as a missing goal-tracking band.
