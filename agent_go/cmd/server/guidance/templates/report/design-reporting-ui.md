Design the workflow's reporting UI from the ground up. **Load `read_skill(skill_name="builder-reference", path="references/report-plan.md")` first and follow it** — it is the canonical report model (a report is HTML document(s) in `reports/report_plan.json` reading `db/db.sqlite` live via `window.report`, no widget grammar) plus the content guide, design-quality bar, theming, and responsive rules. Also load `read_skill(skill_name="builder-reference", path="references/html-output.md")` for the layout + dark-mode skeleton to start from. This skill is the *process*; that reference is the *substance*.{{if .Focus}}
