...
- **`search_web_llm(query, provider, model_id?)`** — Run a live web search. For a published LLM provider, first call `list_llm_capabilities(capability="search_web", include_models=true)`, then pass the matching `provider` and optional `model_id`. For the built-in MCP providers, call it directly with `provider="parallel"`, `provider="exa"`, or `provider="firecrawl"` and omit `model_id`; Parallel and Exa use anonymous free MCP access, while Firecrawl keyless availability is service-controlled. The tool itself has no timeout parameter, and the backend does not impose a fixed ~180-second ceiling on native LLM search. For `codex-cli` specifically, each call launches a real CLI process fresh (no session reuse across calls), and a full search turn — process start, reasoning, searching, and response generation together — has been observed to exceed 180 seconds even though the endpoint was healthy at the time. A timeout at a short self-chosen budget (e.g. ~180s) does not by itself prove the provider is broken — a too-tight budget for a call shape that's simply slower than expected is a real, observed cause. It is not the *only* possible cause: a genuinely stuck/degraded provider is still possible, especially on repeated timeouts. Widen the budget first and retry before concluding the provider is unusable; if timeouts persist across multiple attempts, that's evidence of a real problem, not a budget one. Other CLI-backed providers (`cursor-cli`, `claude-code`, `pi-cli`) likely share the "no built-in timeout" characteristic but have not been separately measured for typical turn latency — don't assume the same ~180s-is-too-short conclusion for them without evidence.
...
- **Giving `search_web_llm` with `provider=codex-cli` a short wait budget (e.g. ~180s)**: a full search turn (process start through response) has been observed to exceed that on a healthy endpoint. A single timeout here doesn't by itself prove the provider is broken — it can simply be a too-tight budget for a call shape that's slower than expected. Widen the budget and retry before switching providers; only treat it as a real provider problem if timeouts persist across multiple widened attempts.
