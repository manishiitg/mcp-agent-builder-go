...
- Writes are picked per step via `knowledgebase_write_method`:
  - `direct` — the **normal/default design choice**. The step agent itself writes notes inline using `diff_patch_workspace_file` for every KB content change, including new topic files and `notes/_index.json` updates. Since PLAT-055/B′ this is verified in the **same merged reflection turn** as learnings (one sequence message covering learnings + KB + concern routing), not a separate KB-only turn. No post-step KB update agent runs for direct-mode steps.
