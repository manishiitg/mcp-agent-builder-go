...
- Writes are picked per step via `knowledgebase_write_method`:
  - `direct` — the **normal/default design choice**. The step agent itself writes notes inline using `diff_patch_workspace_file` for every KB content change, including new topic files and `notes/_index.json` updates. A dedicated post-completion self-review turn fires automatically to verify contribution against the contract. No post-step KB update agent runs for direct-mode steps.
...
- **Path contract — steps MUST use `$DB_PATH`.** A step's shell/Python runs with its working directory set to the step's own execution folder, NOT the workflow root, so a relative `db/db.sqlite` resolves to the wrong place and fails ("unable to open database file") or silently writes a stray empty db. Every step gets an absolute `DB_PATH` env var pointing at the real db — use `sqlite3 "$DB_PATH"` / `os.environ['DB_PATH']`. (You, the builder, run shell at the workflow root, so your own setup/repair `sqlite3 db/db.sqlite` works relatively — but anything you write into a step description or `main.py` must use `$DB_PATH`.) On an open failure, first verify that `DB_PATH` exists, is absolute, and names the workflow db. Never tell a step that Python sqlite is generally sandbox-blocked, switch to the `sqlite3` CLI, emit `.sql` files, or copy the db to `/tmp` as a workaround. If `sqlite3.connect(os.environ['DB_PATH'])` still fails for an existing absolute path, preserve the exact error and report a Runloop runtime/folder-guard failure.
