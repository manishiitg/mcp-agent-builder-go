...
- **Update steps**: `update_scripted_step`, `update_message_sequence_step`, `update_human_input_step`, `update_routing_step`, `update_branch_step`, `update_orchestrator_step`. For a saved legacy `regular` step whose `declared_execution_mode` is absent or non-scripted, call `update_message_sequence_step`; the harness atomically upgrades it to its effective `message_sequence` runtime type while applying the edit. Never use `update_scripted_step` for that compatibility case. The reverse also has an atomic path: a `message_sequence` step whose `declared_execution_mode` is `scripted` (its work is really a checked-in `learnings/{step-id}/main.py`, not a plan-authored message_sequence step) should be edited via `update_scripted_step`, which downgrades its saved type to `regular` while applying the edit — the real scripted executor then reliably injects `$DB_PATH`/`STEP_OUTPUT_DIR`, which the message_sequence runtime does not guarantee. A genuine (non-scripted) `message_sequence` step is rejected by `update_scripted_step` and must keep using `update_message_sequence_step`.
