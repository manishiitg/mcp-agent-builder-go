...
- **Pulse Review+Fix** — Scheduled Pulse stays in one main-agent conversation: Gate → Review+Fix → Finalize. The Review+Fix turn reads Gate's durable worklist, chooses the cheapest sufficient analysis, owns any specialist children, consolidates findings, applies safe fixes, verifies them, and records terminal receipts. Go does not launch reviewer, residual-Fixer, or recovery sessions. Explicit `/engineering-review` follows the same agent-owned operational contract; there is no separate user-facing Fixer command.
