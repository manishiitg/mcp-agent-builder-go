...
- **Pulse Review → Fix** — Scheduled Pulse stays in one main-agent conversation: Gate → Review → Fix → Finalize. Review constructs only the message sequences Gate made due, verifies matured prior issues, conditionally runs Stores Health and Operations, and classifies observations into canonical roots without changing implementation files. A later parent turn launches one fresh independent Fixer for an agent-selected bounded repair batch: one primary coherent bundle plus further independent, low-risk bundles only when each has targeted evidence and its own clear proof boundary. Strategic Review remains read-only and persists its own result. Intermediate turns update one run-scoped checkpoint; the parent validates typed state instead of recreating findings from notification prose. Manual testing follows the same boundary: run `/pulse-review`, then `/pulse-fixer`.
...
**`/pulse-setup` rule**: When setting up recurring Pulse, create or update the recurring execution schedule (`mode="workshop", workshop_mode="run"`, select configured groups/routes as data, leave messages empty) AND one enabled review schedule together, in the same turn: `create_schedule(pulse_review_only=true, cron_expression=..., ...)` — no `group_names`/`route_selections`/`messages` on it, since it never runs the workflow. The enabled review schedule is the single source of truth for recurring Pulse; there is no workflow-level Pulse toggle or mode. Do not create a separate optimizer Goal Advisor schedule; Pulse Gate decides when the Goal Advisor module is due. `/goal-advisor` is a one-off strategy review and must not change schedules.
