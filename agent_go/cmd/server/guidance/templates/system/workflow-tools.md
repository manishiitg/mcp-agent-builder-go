...
- **Pulse Review → Fix** — Scheduled Pulse stays in one main-agent conversation: Gate → Review+Fix → Finalize. Technical maintenance uses one retained executor message sequence. Its review turns are backend-enforced read-only except for the run-scoped checkpoint and typed Pulse records; after it persists a completed `technical_review` receipt and ends that turn, the backend unlocks the later repair turn in the same executor. Strategic Review remains a separate read-only sequence and persists its own result. The parent validates typed state instead of recreating findings from notification prose. Manual `/pulse-review` and every focused `/pulse-review-*` alias use the same receipt-gated retained Review+Fix boundary; `/pulse-fixer` remains an explicit repair-only recovery command for already reviewed issues.
