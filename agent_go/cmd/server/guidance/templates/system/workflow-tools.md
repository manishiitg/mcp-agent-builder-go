...
- **Pulse reviews + Fixer** — Scheduled Pulse runs independent read-only Strategy/Goal reviews, then one shared Engineering/Ops agent performs its Gate-selected review turns, persists the consolidated review, and continues into a bounded Fixer turn in the same conversation. A residual Fixer runs only for still-non-terminal independent or recovery work. Explicit `/engineering-review` launches that same Engineering → Ops → consolidate → Fix sequence; there is no separate user-facing Fixer command.
