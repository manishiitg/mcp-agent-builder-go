...
- **Pulse Review → Fix** — Scheduled Pulse stays in one main-agent conversation: Gate → Review → Fix → Finalize. Review constructs only the message sequences Gate made due, verifies matured prior issues, conditionally runs Stores Health and Operations, and classifies observations into canonical roots without changing implementation files. A later parent turn launches one fresh independent Fixer for at most one coherent canonical repair objective. Strategic Review remains read-only and persists its own result. Intermediate turns update one run-scoped checkpoint; the parent validates typed state instead of recreating findings from notification prose. Manual testing follows the same boundary: run `/engineering-review`, then `/pulse-fixer`.
