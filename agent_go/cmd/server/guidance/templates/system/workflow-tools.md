...
- **Pulse Review+Fix** — Scheduled Pulse stays in one main-agent conversation: Gate → Review+Fix → Finalize. Review+Fix constructs only the message sequences Gate made due. The Engineering sequence verifies matured prior issues before discovery, conditionally runs Stores Health and Operations turns, fixes and verifies retained roots, then persists typed state in its final turn. Strategic Review combines current-strategy criticism with conditional outside-the-plan opportunity discovery and persists in its own final turn. Intermediate turns continuously update one run-scoped checkpoint; there is no default consolidation turn. The parent validates terminal typed receipts instead of recreating findings from auto-notification prose. Go does not select or launch reviewer, residual-Fixer, or recovery sessions. Explicit `/engineering-review` follows the same agent-owned operational contract; there is no separate user-facing Fixer command.
...
**`/pulse-setup` rule**: When setting up recurring Pulse, create or update the recurring execution schedule (`mode="workshop", workshop_mode="run"`, select configured groups/routes as data, leave messages empty) AND the review schedule together, in the same turn: `create_schedule(pulse_review_only=true, cron_expression=..., ...)` — no `group_names`/`route_selections`/`messages` on it, since it never runs the workflow — then `update_workflow_config(post_run_monitor=true, post_run_monitor_mode="periodic")`. Never create the run schedule without the review schedule, or vice versa: a workflow left in `"periodic"` mode with no review schedule yet gets a lightweight pass every run and a full review from nothing, ever. Do not create a separate optimizer Goal Advisor schedule; Pulse Gate decides when the Goal Advisor module is due. `/goal-advisor` is a one-off strategy review and must not change schedules or the Pulse toggle.

**Pulse never runs inline with a normal scheduled run.** `post_run_monitor_mode="periodic"` is mandatory for every workflow, not a frequency-based judgment call — a normal scheduled run does backup, publish, and notify only; Gate/Review+Fix/Finalize run separately, on the review schedule's own cadence, over whatever run backlog has accumulated. This applies regardless of how often the workflow runs: long Pulse-adjacent sessions reused across runs have caused real reliability problems independent of run frequency, so `per_run` mode is not an option to choose here.

Read this workflow's intended run frequency (or, for an existing workflow, its actual `get_schedule_runs` history) to choose the review schedule's INTERVAL — that remains a judgment call, balancing review latency against genuinely batched evidence; when in doubt, prefer more frequent over less, since Gate's own backlog reasoning already handles reviewing several accumulated runs in one pass cheaply. Check `run_retention_count` (workflow.json, default 5) against the interval chosen: if the workflow could produce more runs between reviews than retention preserves, raise it, since a rotated run folder beyond `run_retention_count` is permanently deleted. The lightweight per-run pass itself keeps re-checking this cadence on an ongoing basis as actual run volume changes — this is the one-time setup version of that same responsibility, not the only time it happens.
