...
**Periodic vs per-run Pulse**: `post_run_monitor=true` alone means Gate/Review+Fix/Finalize run after every scheduled run, in that run's own session — the right default for a workflow that runs a few times a day or less. For a workflow intended to run frequently (hourly or more), consider `post_run_monitor_mode="periodic"` instead: every run gets only a lightweight backup+notify pass, and a *separate* schedule reviews the accumulated backlog on its own cadence. Long Pulse-adjacent sessions reused across frequent runs have caused real reliability problems, so this is a genuine tradeoff, not a strict upgrade — a real problem now waits up to the review interval before anything catches it. Decide from the intended run frequency, not by default:

- If periodic mode fits, create both schedules together: the normal run schedule, and a second one with `create_schedule(pulse_review_only=true, cron_expression=..., ...)` — no `group_names`/`route_selections`/`messages` on the review schedule, since it never runs the workflow. Then call `update_workflow_config(post_run_monitor_mode="periodic")` on the same turn the review schedule is created, never before — a workflow left in `"periodic"` mode with no review schedule yet gets a lightweight pass every run and a full review from nothing, ever.
- If the workflow runs rarely enough that per-run Pulse was never going to cost much, leave `post_run_monitor_mode` unset (`per_run`, the default). Say so explicitly rather than silently defaulting — a builder agent choosing `per_run` because it genuinely fits is different from one that never considered the choice.
