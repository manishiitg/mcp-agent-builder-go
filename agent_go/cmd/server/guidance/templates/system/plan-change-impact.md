...
- **Report dashboard** — search `reports/report_plan.json` and the dashboard HTML's `window.report.query` SQL for the db tables/columns and output fields. A query that reads changed data must be fixed, and `report_plan.json` updated. (`read_skill(skills=[{"name":"builder-reference","path":"references/report-plan.md"}])`)
