...
2. Use `pulse_run_id="current"`. Read the saved Gate worklist and
   `get_pulse_state(view="backlog", detail="compact")` exactly once. Select a
   bounded repair batch from that index, then request `detail="full"` only for
...
3. Select a bounded canonical **repair batch**. Start with the highest-value
...
4. Apply and verify each selected bundle before starting the next. Use the
...
6. Finish with a concise statement of the selected repair batch: bundles and
