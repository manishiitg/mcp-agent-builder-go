...
Call read_skill(skill_name="builder-reference", path="references/chief-task-report.md") and follow it exactly.
...
func postRunMonitorGateStep(pulseRunID, runFolder, runStatus string) postRunMonitorStep {
	return postRunMonitorStep{
		label: "gate",
		query: fmt.Sprintf("PULSE GATE / WORKLIST. pulse_run_id=%q, run_folder=%q, run_status=%q. Load read_skill(skill_name=\"builder-reference\", path=\"references/pulse-gate.md\") and follow it exactly. Perform only the progressive Gate scan, call record_pulse_worklist exactly once with all %d module decisions, and stop. Do not write `builder/improve.html` or any workflow artifact; do not launch reviewers, fix, back up, publish, or notify.",
