...
func pulseLifecycleAgenticReviewStep(pulseRunID string) pulseLifecycleStep {
	...
	return pulseLifecycleStep{
		...
		Use run_in_background to dispatch at most two ordinary background reviewers. The Technical reviewer is one message sequence, not several agents: (1) perform a lightweight safety scan from get_pulse_state(view="backlog", detail="compact") and the focus agenda; (2) choose exactly one deep technical focus using the persisted lifecycle priority and rotation rules; (3) request full detail only for selected public PUL ids and authoritative evidence selectors; (4) verify matured retained findings and investigate the selected focus, loading Stores Health or Operations evidence only when that focus needs it; (5) classify every selected workflow observation by linking it to an existing issue, promoting it, or rejecting it with evidence; (6) record the technical_review focus and persist typed findings, verification evidence, and exactly one terminal technical_review receipt. It must not modify workflow/platform implementation files. Every message reads and updates the same compact checkpoint at %q. It continuously merges semantic duplicates into canonical root causes; do not add a separate consolidation turn. In backlog_drain restrict it to matured verification and classification of retained evidence—no broad discovery.

		The Strategic reviewer is one message sequence for the single strategic_review module: (1) perform the same lightweight safety/evidence scan and choose exactly one strategic focus from its focus agenda; (2) verify retained strategic findings and audit the selected hidden mechanism such as feedback loops, selection bias, observation contamination, proxy optimization, concentration, saturation, local optima, goal measurement, or experiment impact; (3) only when Gate evidence warrants opportunity discovery, independently search for materially different approaches; (4) critic/conclusion with exactly one of keep, improve, propose_alternative, experiment, or evidence_wait; (5) record the strategic_review focus, then persist typed findings, decisions, impact records, and exactly one terminal strategic_review receipt. Every turn reads and updates %q. Audit-only and backlog_drain omit opportunity discovery. An experiment is optional, and concurrent running experiments are permitted only when their declared interference domains do not overlap.
