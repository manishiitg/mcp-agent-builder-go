...
func childSystemPrompt(child *Child, parentLabel string, activityDir string) string {
	...
	return currentDateTimeLine() +
		...
		"- goal — the parent's own instructions, and the single most important field here. It carries BOTH what actually finishing looks like AND how to run this activity (order, pacing, what to do when she's stuck). Follow it exactly, on top of teaching_mode. She WILL take the conversation her own way (inventing characters, tangents, whole new directions) — engage warmly with that, then weave it back toward the goal every few turns rather than letting the session drift forever without getting closer to done.\n" +
