...
// toolOnlyArtifactWriteGuards maps a workspace file that may only change through
// a tool to the tool that owns it.
var toolOnlyArtifactWriteGuards = map[string]string{
	evaluationPlanRelPath: "update_evaluation_plan",
}

// guardToolOnlyArtifactWrites refuses a generic file write aimed at an artifact
// that has a dedicated tool.
//
// The kernel sandbox already denies these paths for shell commands, but that
// only covers one of two write surfaces: typed workspace tools like
// diff_patch_workspace_file go through the server, not the sandbox, and would
// otherwise still edit the file with no changelog entry. Guarding one surface
// and calling the file protected is the kind of half-enforcement that reads as
// safe and is not.
//
// The refusal names the tool to use, because an agent that hits a bare
// permission error will look for another way to write the file rather than for
// the tool it should have used.
func guardToolOnlyArtifactWrites(toolName string, executor interface{}) interface{} {
	if toolName != "diff_patch_workspace_file" && toolName != "update_workspace_file" {
		return executor
	}
	typed, ok := executor.(func(context.Context, map[string]interface{}) (string, error))
	if !ok {
		return executor
	}
	return func(ctx context.Context, args map[string]interface{}) (string, error) {
		for _, key := range []string{"filepath", "file_path", "path"} {
			...
			for artifact, owner := range toolOnlyArtifactWriteGuards {
				if strings.HasSuffix(candidate, artifact) {
					return "", fmt.Errorf(
						"%s cannot be edited with %s; use %s so the change is recorded in planning/changelog with its reason. Editing it directly leaves artifact drift review unable to see the change",
						artifact, toolName, owner,
					)
				}
			}
		}
