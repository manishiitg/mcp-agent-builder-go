...
These tools cover each provider's **basic** parameters only. For a capability a tool doesn't expose (e.g. multi-image composition, conversational editing, voice cloning), call ` + "`read_skill(skills=[{\"name\":\"builder-reference\",\"path\":\"references/workspace-media-tools.md\"}])`" + ` — it covers writing Python against the provider directly, including the credential gap (` + "`set_provider_auth`" + ` keys aren't shell-visible; use ` + "`set_workflow_secret`" + `/` + "`set_user_secret`" + ` instead).

**For the full reference — every tool's parameters, defaults, provider routing rules, model-ID lists, and common-mistake gotchas — call:** ` + "`read_skill(skills=[{\"name\":\"builder-reference\",\"path\":\"references/workspace-media-tools.md\"}])`" + `.`
}
...
Provider-backed text, web search, image, video, audio, music, and media-reading tools are available through the MCP bridge. Before selecting a provider/model, handling provider auth, or using one of these tools, read the attached ` + "`builder-reference`" + ` skill's ` + "`references/workspace-media-tools.md`" + ` (or call ` + "`read_skill(skills=[{\"name\":\"builder-reference\",\"path\":\"references/workspace-media-tools.md\"}])`" + `). Use ` + "`list_llm_capabilities`" + ` as the authoritative availability source and ` + "`set_provider_auth`" + ` for credentials; never place credentials in shell commands or files.`
}
