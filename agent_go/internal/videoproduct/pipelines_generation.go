...
// Kept in one place so the three stages that spend cannot drift apart on what
// counts as approval, and asserted by test against RequiresApproval so the
// flag cannot become decorative.
//
...
var longformStageDescriptions = map[string]string{
	...
	"longform-narration": "Generate the narration, because at this length the voiceover is the spine the visuals are cut to — deriving shot durations from an estimate instead of from measured audio is the most expensive mistake in this pipeline, forcing either regenerated clips or a rewritten script. " +
		"Read longform-script.md and follow video-editing's narration guidance. Generate the voiceover in per-beat or per-chapter segments, never as one long pass, so a single bad beat can be regenerated later without re-timing everything after it. Use fal-ai for TTS unless longform-brief.md commits to something else. " +
...
var longformPipeline = &Pipeline{
	...
	Stages: []PipelineStage{
		...
		{ID: "longform-characters", Title: "Define characters", Summary: "Lock every recurring subject's spec and reference image before any shot exists.", Output: "longform-characters.md", Skills: []string{"video-cinematography", "video-model-selection", "fal-ai", "google-ai"}, RequiresApproval: true},
		{ID: "longform-narration", Title: "Generate narration", Summary: "Produce the voiceover in segments and measure its real duration.", Output: "longform-narration.md", Skills: []string{"video-editing", "fal-ai", "video-storytelling"}, RequiresApproval: true},
		...
		{ID: "longform-generate", Title: "Generate shots", Summary: "Produce the approved shots, conditioned on each character's reference.", Output: "longform-generation.md", Skills: []string{"fal-ai", "google-ai", "video-cinematography", "video-model-selection"}, RequiresApproval: true},
...
var shortformPipeline = &Pipeline{
	...
	Stages: []PipelineStage{
		...
		{ID: "shortform-generate", Title: "Generate shots", Summary: "Produce the approved shots.", Output: "shortform-generation.md", Skills: []string{"fal-ai", "google-ai", "video-cinematography", "video-model-selection"}, RequiresApproval: true},
