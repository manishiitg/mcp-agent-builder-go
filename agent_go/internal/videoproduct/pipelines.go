...
const h3ReferenceWorkflowInvariant = "\n\nVideo Studio route invariant: every paid generated-video clip uses Fal's MiniMax H3 Max only at default `resolution: \"480P\"` with `prompt_expansion_mode: \"balanced\"`. Use `768P` only when the user explicitly requests and approves its higher cost. Use `minimax/h3-max/text-to-video` only for a prompt-only shot with no approved visual or continuity reference; use `minimax/h3-max/image-to-video` when an approved first frame must control the shot, optionally with `end_image_url` for an explicit ending frame; and use `minimax/h3-max/reference-to-video` for identity, style, audio, motion, or continuation, sending the accepted immediate predecessor in `reference_video_urls` as Video 1. A first/last-frame seam bridge is allowed only after a direct-cut seam proof visibly fails. Do not choose standard H3, another provider, model, H3 Max route, or resolution. Never use a hard cut, crossfade, or a bridge to conceal unrelated clips; a failed seam remains failed until the replacement boundary passes review."
