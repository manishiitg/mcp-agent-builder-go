...
const h3ReferenceWorkflowInvariant = "\n\nVideo Studio route invariant: every paid generated-video clip uses Fal's MiniMax H3 Max only at default `resolution: \"480P\"` with `prompt_expansion_mode: \"balanced\"`. Use `768P` only when the user explicitly requests and approves its higher cost. Anchors and normal continuations use `minimax/h3-max/reference-to-video`: send approved reference images and, for a continuation, the accepted immediate predecessor in `reference_video_urls` as Video 1. A seam bridge is the single exception: only after a direct-cut seam proof visibly fails, use `minimax/h3-max/image-to-video` with its verified `image_url` and `end_image_url` boundary-frame fields to generate a reviewable bridge. Do not choose standard H3, another provider, model, H3 Max route, or resolution. Never use a hard cut, crossfade, or a bridge to conceal unrelated clips; a failed seam remains failed until the replacement boundary passes review."
